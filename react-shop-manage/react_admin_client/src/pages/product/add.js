...
import {Card,Icon,Form,Input,Cascader,Button,message,BackTop,InputNumber  } from 'antd'
...
class ProductAdd extends Component {
  ...
  constructor(props){
    ...
    const {state} = this.props.location //修改和添加其实为一个页面，利用{state}控制显示
    this.title=(
  ...
  getProductClass= async (parentId)=>{
    ...
    if(parentId==='0'){ //一级列表
      this.initSelectOptions(data)//初始化option
      return
    }
    return data
  }

  productLoadData= async (selectedOptions)=>{//获得点击option的子option
    const targetOption = selectedOptions[selectedOptions.length - 1];//option默认可以选中多个
    ...
    } else {//如果没有子option，则覆盖option为叶子option
  ...
  render() {
    let prdCategory=[]
    const oldData = this.props.location.state || {}//修改，接受传来的state
...
export default Form.create()(withRouter(ProductAdd));