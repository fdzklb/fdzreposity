...
const filterRecursive = filter => {
  ...
  for (let i = 0; i < filter.length; i++) {
    if (filter[i].type === 'atom') {
      ...
      filter[i].filters.forEach(e => {
        e['logic'] = filter[i]['logic'];
      });
  ...
  switch (operation) {
    case 'select':
      ...
      if (limit) {
        ...
        sql =
          sql + ' LIMIT ' + (page - 1) * limit.pageSize + ',' + limit.pageSize;
      }
    ...
    case 'insert':
      ...
      if (protocol.fields && protocol.values) {
        for (let i = 0; i < protocol.fields.length; i++) {
          tmpField += protocol.fields[i];
          tmpValue += '?';
          sqlParams = sqlParams.concat([protocol.values[i]]);

          if (i !== protocol.fields.length - 1) {
            tmpField += ', ';
            tmpValue += ', ';
          }
        }
